...
let make = (
  ~tabs: tabs, 
  ~currentTab : string, 
  ~onTabSelectCb: (string) => _,
  ~onTabCloseCb: (string) => _
) => {
				...
				(
					...
					|> List.map((tab: tab) => {
						currentTab === tab.label ?
							<div key={tab.label} className="display-tab" style=(ReactDOMRe.Style.make(~display="block", ()))> 
								tab.component 
							</div>
						: 
							<div key={tab.label} className="no-display-tab" style=(ReactDOMRe.Style.make(~display="none", ()))> 
								tab.component 
							</div>
						}
					)
