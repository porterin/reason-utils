...
let getAccordionHeader = (expanded: bool, panel: t) => {
  <MaterialUi.AccordionSummary className={"header " ++ (expanded ? "expanded-header" : "")}>
    <Row>
      {expanded
         ? <Icon.ExpandLessIcon className="expand-icon expand-less-icon" fontSize=`Large />
         : <Icon.ExpandMoreIcon className="expand-icon expand-more-icon" fontSize=`Large />}
      {switch (panel.header) {
